<div class="sidebar-profile-section">
    {{-- User Avatar --}}
    <div class="sidebar-profile-avatar-wrap">
        @if(auth()->check() && auth()->user()->staffProfile?->profile_photo)
            <div class="sidebar-avatar">
                <img src="{{ asset('storage/' . auth()->user()->staffProfile->profile_photo) }}" alt="{{ auth()->user()->name }}">
            </div>
        @else
            <div class="sidebar-avatar">
                <svg class="sidebar-avatar-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
            </div>
        @endif
    </div>

    {{-- User Name --}}
    <div class="sidebar-user-text sidebar-user-name">
        {{ auth()->check() ? auth()->user()->name : 'Guest User' }}
    </div>

    {{-- User Email --}}
    <div class="sidebar-user-text sidebar-user-email">
        {{ auth()->check() ? auth()->user()->email : 'guest@example.com' }}
    </div>

    {{-- Role Badge --}}
    <div class="sidebar-role-wrap">
        <div class="sidebar-user-text sidebar-role-badge">
            <svg fill="currentColor" viewBox="0 0 20 20">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
            </svg>
            <span>
                @if(auth()->check())
                    @if(auth()->user()->staffProfile?->designation)
                        {{ auth()->user()->staffProfile->designation }}
                    @elseif(auth()->user()->isAdmin())
                        Administrator
                    @else
                        Staff Member
                    @endif
                @else
                    Guest
                @endif
            </span>
        </div>
    </div>
</div>
